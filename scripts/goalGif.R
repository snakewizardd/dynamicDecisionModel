...
decision_log <- decision_log %>%
  mutate(adjusted_goal_wrapped = str_wrap(adjusted_goal, width = 40)) 

# Base plot
p <- ggplot(decision_log, aes(x = iteration, y = iteration)) +
  ...
  labs(title = "Goal Progression Over Iterations",
       ...
       y = "Progression") +
  theme_minimal() +
  annotate("text", x = 1, y = max(decision_log$iteration) + 1, 
           label = paste("Original Goal:\n", str_wrap(unique(decision_log$original_goal), 50)), 
...
# Save animation
anim_save("goal_progress.gif", animated_plot)
