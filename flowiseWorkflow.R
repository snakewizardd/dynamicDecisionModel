...
InfoRow <- rbind(data.frame(Row = paste0('Old Goal: ',iterationRow[1,'pnull'])),
data.frame(Row = paste0('Original Information: ',iterationRow[1,'I1'])),
data.frame(Row = paste0('Choices Considered: ',iterationRow[1,'D'])),
data.frame(Row = paste0('Choice Chosen: ',iterationRow[1,'Fn'])),
data.frame(Row = paste0('New Information From Run: ',iterationRow[1,'Idelta'])))
...
####Need to figure out how to continue looping and filling the iterationRow as well as updating the InfoRow

#This begins the second loop content
reset_messages()
addUserMessage(userMessage = newInformationContext)
pingAPI(userInputMessage = newInformationContext,agentType = 'Semantic')
